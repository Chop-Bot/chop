...
const getServersThatChanged = (oldServers, newServers) => {
  const changed = [];
...
const getCurrentAndOldStatus = async () => {
  let current = null;
  try {
...
module.exports = class extends Task {
  ...
  async run() {
    ...
    const [currentStatus, oldStatus] = await getCurrentAndOldStatus();
    if (!currentStatus || !oldStatus) return;

    // check if there is change in any of the servers, return if not
    const serversThatChanged = getServersThatChanged(
      oldStatus.servers,
    ...
    const notificationSettings = await Notification.statusByServers(serversThatChanged);
    notificationSettings.forEach(n => guildsToNotify.set(n.guild, n.teraStatus.channel));
